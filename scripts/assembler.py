...
Two-operand (reg, immediate):
  addi rt, imm  -- rt = rt + imm  [imm: signed 2-bit, range -2..1]
...
-------------------------------------
  ...
  0001aatt  sub
  0010iitt  addi     (ii = signed 2-bit two's-complement: 00=0,01=1,10=-2,11=-1)
  0011aatt  cpy
...
------------
  ...
  ; r0 = scratch (reserved), r1 = counter
...
def instruction_size(tokens):
    """Return 3 for a label-targeting jz/js; 1 for everything else."""
...
def encode(tokens, line_no, label_table):
    ...
    # --- addi rt, imm2 (signed -2..1) ---
    if op == 'addi':
        expect(2)
        tt  = parse_reg(tokens[1], line_no)
        imm = parse_imm(tokens[2], -2, 1, line_no, 'imm2 (−2..1)')
        ii  = imm & 0x3     # 2-bit two's-complement
        return [(f'addi r{tt}, {imm}', 0x20 | (ii << 2) | tt)]
